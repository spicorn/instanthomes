...
const ContactInfo = () => (
  <section className="relative py-10 sm:py-16 px-2 sm:px-4 md:px-8 w-full animate-fade-in-up" id="contact-info">
    ...
    <motion.div
      ...
      className="relative z-10 max-w-4xl lg:max-w-5xl mx-auto bg-white/80 backdrop-blur-2xl rounded-3xl shadow-2xl border-2 border-blue-200/40 p-4 sm:p-8 lg:p-16 flex flex-col gap-8 items-center overflow-hidden group"
    >
      ...
      <h2 className="relative z-10 text-2xl sm:text-3xl font-extrabold text-primary mb-2 drop-shadow-xl tracking-tight text-center flex items-center gap-3">
        <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-blue-400 shadow-lg border-4 border-white text-white text-xl animate-bounce">
          <FaEnvelope />
        </span>
        Contact Information
      </h2>
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
        {infoItems.map((item, i) => (
          <motion.div
            ...
            viewport={{ once: true, amount: 0.2 }}
            className="relative z-10 flex items-center gap-1 sm:gap-2 bg-white/90 rounded-2xl px-4 py-4 sm:px-6 sm:py-5 shadow-md hover:shadow-2xl transition-all duration-300 border border-blue-100 group-hover:bg-blue-50/80 text-base sm:text-lg font-semibold lg:text-xl w-full max-w-full"
            whileHover={{ scale: 1.03, boxShadow: '0 8px 32px #68AAFF22', backgroundColor: '#f0f7ff' }}
            whileTap={{ scale: 0.98 }}
          >
            <span className="text-xl sm:text-2xl animate-pulse">{item.icon}</span>
            <span className="text-blue-800 min-w-[70px] sm:min-w-[90px] font-bold break-words whitespace-nowrap">{item.label}:</span>
            <span className="text-blue-700 flex-1 break-words whitespace-pre-line">{item.value}</span>
          </motion.div>
        ))}
      </div>
    </motion.div>
