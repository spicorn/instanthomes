...
import { FaFlag, FaMoneyBillWave, FaClock, FaGlobeAfrica } from 'react-icons/fa'

const reasons = [
  { icon: <FaFlag />, title: 'Australian Quality', desc: 'Adapted from top Australian prefab standards.' },
  { icon: <FaMoneyBillWave />, title: 'Affordable', desc: 'Competitive pricing for every budget.' },
  { icon: <FaClock />, title: 'Fast Turnaround', desc: 'Move in quickly with our efficient process.' },
  { icon: <FaGlobeAfrica />, title: 'Local Expertise', desc: 'Built for Zimbabwe, by Zimbabweans.' },
]
...
const WhyChooseUs = () => (
  <motion.section
    ...
    className="py-16 sm:py-28 px-2 sm:px-6 md:px-12 w-full bg-gradient-to-br from-blue-50 via-white to-blue-100" id="why"
  >
    <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-center mb-14 text-primary drop-shadow-xl tracking-tight">Why Choose Us</h2>
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 max-w-7xl mx-auto">
      {reasons.map((reason, i) => (
        ...
        >
          ...
          <h3 className="text-xl md:text-2xl font-bold text-primary mb-2 drop-shadow-sm tracking-tight">{reason.title}</h3>
          <p className="text-dark/80 text-base md:text-lg leading-relaxed font-medium max-w-xs mx-auto">{reason.desc}</p>
        </motion.div>
      ))}
    </div>
  </motion.section>
